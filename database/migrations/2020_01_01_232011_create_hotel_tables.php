...
class CreateHotelTables extends Migration
{
    ...
    public function up()
    {
        Schema::create('room_types', function (Blueprint $table) {
            ...
            $table->string('name', 255)->comment('Tje Name of The room type.ie Double Queen Single King etc');
            $table->text('description')->comment('Full text descrion of the room type.');
            $table->timestamps();
            $table->softDeletes();
        });
        ...
        Schema::create('discounts', function (Blueprint $table) {
            $table->bigIncrements('id')->comment('The primary key for the the table.');
            $table->string('name', 255);
            $table->string('code')->comment('The code some one would expect to enter at checkout.');
            $table->unsignedBigInteger('discount')->comment('The discount in whole cents for a room.');
            $table->timestamps();
            $table->softDeletes();
        });
        Schema::create('rates', function (Blueprint $table) {
            $table->bigIncrements('id')->comment('The primary key for the the table.');
            $table->unsignedBigInteger('value')->comment('The rate for the room in whole cents.');
            $table->unsignedBigInteger('room_type_id')->index('room_type_id')->comment('The corresponding room type.');
            $table->boolean('is_weekend')->default(false)->comment('If this is not the weekend rate or not.');
            $table->timestamps();
    ...
    {
        Schema::dropIfExists('bookings_user');
